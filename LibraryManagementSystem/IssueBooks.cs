...
namespace LibraryManagementSystem
{
    ...
    {
        SqlConnection connect = new SqlConnection(@"Data Source=WIN-1B93A5H1P7S\SQLEXPRESS;Initial Catalog=Biblioteca;Integrated Security=True;Connect Timeout=30");
        public IssueBooks()
        {
            InitializeComponent();
        ...
        private void bookIssue_addBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Completati toate spatiile libere", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if(connect.State != ConnectionState.Open)
                {
                    try
                    {
                        ...
                        string insertData = "INSERT INTO issues " +
                            "(issue_id, full_name, grupa, email, book_title, author, status, issue_date, return_date, date_insert) " +
                            "VALUES(@issueID, @fullname, @grupa, @email, @bookTitle, @author, @status, @issueDate, @returnDate, @dateInsert)";
                        ...
                        using(SqlCommand cmd = new SqlCommand(insertData, connect))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@grupa", bookIssue_contact.Text.Trim());
                            ...
                            MessageBox.Show("Emitere cu succes!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void bookIssue_updateBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (connect.State != ConnectionState.Open)
                {
                    DialogResult check = MessageBox.Show("Sunteti siguri ca doriti sa faceti UPDATE la Emitere ID:"
                    ...
                    if (check == DialogResult.Yes)
                    {
                        try
                        {
                            connect.Open();
                            DateTime today = DateTime.Today;
                            string updateData = "UPDATE issues SET full_name = @fullName, grupa = @grupa, email = @email" +
                            ...
                            using (SqlCommand cmd = new SqlCommand(updateData, connect))
                            {
                                ...
                                MessageBox.Show("Azctualizare cu succes!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void bookIssue_deleteBtn_Click(object sender, EventArgs e)
        {
            if (bookIssue_id.Text == ""
                ...
                || bookIssue_picture.Image == null)
            {
                MessageBox.Show("Selectati obiectul", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if (connect.State != ConnectionState.Open)
                {
                    DialogResult check = MessageBox.Show("Sunteti siguri ca doriti sa faceti DELETE la Emitere ID:"
                    ...
                    if (check == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(updateData, connect))
                            {
                                ...
                                MessageBox.Show("Stergere cu succes!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
